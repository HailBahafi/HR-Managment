...
interface Props { params: Promise<{ department: string; requestType: string }> }
...
export default function DepartmentRequestTypePage({ params }: Props) {
    ...
    return (
      <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          لم يُعثر على نوع طلب بالمسار: <span dir="ltr" className="font-mono text-xs">{department}/{requestType}</span>
        </p>
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-card p-6 shadow-soft space-y-2">
        <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.15em] text-muted-foreground">
          {rt.departmentId === HR_REQUEST_TYPE_ALL_DEPARTMENTS_ID
            ? 'جميع الأقسام'
            : dept?.nameAr ?? department}
      ...
      <div className="flex gap-3 rounded-xl border border-primary/20 bg-primary/5 p-5">
        ...
        <div className="space-y-1">
          <p className="font-semibold text-sm">قائمة الطلبات قيد التطوير</p>
          <p className="text-sm text-muted-foreground">
            ستظهر هنا قائمة الطلبات المرتبطة بهذا النوع عند ربط النظام بواجهة برمجية (API).
            في الوقت الحالي، استخدم صفحة{' '}
            <Link href="/hr/requests/general" className="text-primary hover:underline">الطلبات العامة</Link>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {activeSubtypes.map(sub => (
              <span key={sub.id} title={hrRequestPath(department, sub.slug)}
                className="inline-flex items-center rounded-full border border-border bg-card px-3 py-1.5 text-sm font-medium shadow-soft hover:border-primary/40 hover:bg-primary/5 transition-colors cursor-default">
                {sub.nameAr}
              </span>
            ))}
          </div>
