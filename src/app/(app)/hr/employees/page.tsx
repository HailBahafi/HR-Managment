            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((emp) => {
                  ...
                  const branch = getBranch(emp.branchId);
                  return (
                    <tr key={emp.id} className="group border-b border-border/60 transition-colors hover:bg-muted/20 last:border-b-0 cursor-pointer" onClick={() => router.push(`/hr/employees/${emp.id}`)}>
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/hr/employees/${emp.id}`} className="text-xs font-medium text-primary opacity-0 transition-opacity group-hover:opacity-100">عرض →</Link>
                      </td>
                    </tr>
                  );
                })}
