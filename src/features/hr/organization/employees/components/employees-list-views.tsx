...
export function EmployeesListViews({ model }: Props) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {view === 'table' ? (
        <EmployeesListTable filtered={filtered} router={router} />
      ) : (
...
function EmployeesListTable({
  ...
  filtered: EmployeesListModel['filtered'];
        ...
        <table className="w-full">
          ...
          <tbody>
            {filtered.map((emp) => {
              ...
              return (
                <tr key={emp.id} className="group border-b border-border/60 transition-colors hover:bg-muted/20 last:border-b-0 cursor-pointer" onClick={() => router.push(hrOrganizationRoutes.employee(emp.id))}>
                  <td className="px-6 py-4">
                    <Link href={hrOrganizationRoutes.employee(emp.id)} className="flex items-center gap-3">
                  ...
                  <td className="px-6 py-4">
                    <Link href={hrOrganizationRoutes.employee(emp.id)} className="text-xs font-medium text-primary opacity-0 transition-opacity group-hover:opacity-100">عرض →</Link>
                  </td>
                </tr>
              );
            })}
...
function EmployeesListGrid({ filtered }: { filtered: EmployeesListModel['filtered'] }) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {filtered.map((emp) => {
        ...
        const branch = getBranch(emp.branchId);
        return (
          <Link key={emp.id} href={hrOrganizationRoutes.employee(emp.id)}
