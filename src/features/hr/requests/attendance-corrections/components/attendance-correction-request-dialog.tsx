...
function emptyPeriod(): CorrectionFormPeriod {
  return {
    periodId: 'period-1',
    labelAr: 'الوردية',
    expectedRangeAr: '—',
...
export function AttendanceCorrectionRequestDialog({
  ...
  const [formPeriods, setFormPeriods] = React.useState<CorrectionFormPeriod[]>([emptyPeriod()]);
  ...
  const updatePeriod = React.useCallback(
    (index: number, patch: Partial<CorrectionFormPeriod>) => {
      setFormPeriods((prev) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasCorrectedPunch = apiPeriods.some(
      (p) => p.corrected.checkInAt || p.corrected.checkOutAt,
    );
    if (!hasCorrectedPunch) {
      toast.error('أدخل وقت حضور أو انصراف مصحّحاً لفترة واحدة على الأقل.');
      return;
    }
      ...
      <DialogContent className={cn(dialogShellContentClass, 'sm:max-w-xl')} dir="rtl">
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <DialogHeader className={dialogShellHeaderClass}>
            ...
            <DialogDescription>
              تُحمَّل فترات اليوم من سجل الحضور؛ عدّل الأوقات المصحّحة ثم أرسل الطلب.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogBody className={cn(dialogShellBodyClass, 'grid gap-4')}>
            ...
            {!breakdownLoading && formEmpId && formWorkDate && breakdown?.periods.length === 0 ? (
              <p className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-amber-800">
                لا توجد فترات دوام مسجّلة لهذا اليوم — يمكنك إدخال فترة واحدة يدوياً.
              </p>
            ) : null}
            ...
            <div className="space-y-3">
              {formPeriods.map((period, index) => (
                ...
                >
                  ...
                  <div className="rounded-lg border border-border/50 bg-background/70 px-3 py-2 text-xs">
                    <p className="mb-1 font-medium text-muted-foreground">مسجّل</p>
                    <p className="font-mono tabular-nums" dir="ltr">
                      دخول {formatClockLabel(
                        timePickerToIso(formWorkDate, period.recordedCheckIn, timezoneOffsetMinutes),
                        timezoneOffsetMinutes,
                      ) || '—'}
                      {' · '}
                      خروج {formatClockLabel(
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label className="text-xs font-medium">تصحيح — حضور</Label>
                      <ModernTimePicker
                        value={period.correctedCheckIn}
                        onChange={(v) => updatePeriod(index, { correctedCheckIn: v })}
                        placeholder="وقت الدخول"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs font-medium">تصحيح — انصراف</Label>
                      <ModernTimePicker
                        value={period.correctedCheckOut}
                        onChange={(v) => updatePeriod(index, { correctedCheckOut: v })}
          ...
          <DialogFooter className={dialogFormFooterClass}>
            <Button type="submit" variant="luxe" disabled={submitting || breakdownLoading}>
