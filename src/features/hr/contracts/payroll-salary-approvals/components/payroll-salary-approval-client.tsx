...
import { formatLatinNumber } from '@/features/hr/contracts/lib/compensation-preview';
import {
  hrContractsPayrollSalaryApprovalsQueryHref,
  hrContractsPeriodCompensationHref,
} from '@/features/hr/contracts/constants/routes';
...
import { PayslipDetailDialog } from '@/features/hr/contracts/payroll-salary-approvals/components/payslip-detail-dialog';
...
export function PayrollSalaryApprovalClient() {
  ...
  const notificationDefaults = React.useMemo(() => {
    ...
    return {
      ...
      actionUrl: hrContractsPayrollSalaryApprovalsQueryHref(period.id),
      ...
      {period && (
        <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-muted/15 px-3 py-2.5">
          ...
          <Button variant="ghost" size="sm" className="h-7 gap-1 px-2 text-[11px] text-primary" asChild>
            <Link href={hrContractsPeriodCompensationHref(period.id)}>
